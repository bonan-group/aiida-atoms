...
class VaspElasticWorkChain(WorkChain, WithBuilderUpdater):
    ...
    def define(cls, spec):
        super().define(spec)
        spec.input('elastic_settings', valid_type=orm.Dict, 
                   required=False,
        ...
        spec.output('relaxed_structure', valid_type=orm.StructureData, help='The fully relaxed structure after the full relaxation step')
        spec.output('primitive_structure', valid_type=orm.StructureData, help='The standardized primitive structure or unit cell after the full relaxation step')
    ...
    def setup(self):
        ...
        if pdict != self.ctx.relax_inputs.vasp.parameters.get_dict():
            self.logger.warn('The VASP parameters have been modified to remove `ibrion`, `isif`, and `nsw` settings for the elastic tensor calculation.')
    ...
    def standardize(self):
        ...
        if primitive_type == 'conventional':
            ...
            conventional_structure = SpacegroupAnalyzer(relaxed_structure.get_pymatgen()).get_conventional_standard_structure()
            ...
            prim_atoms_data = spglib.find_primitive((atoms.cell, atoms.get_scaled_positions(), atoms.numbers), 
                                                    symprec=self.ctx.elastic_settings.get('symprec', 1e-3))
            primitive_lat, primitive_pos, primitive_numbers = prim_atoms_data
            primitive_atoms = ase.Atoms(cell=primitive_lat, scaled_positions=primitive_pos, numbers=primitive_numbers, pbc=True)
            self.ctx.reference_structure = orm.StructureData(ase=primitive_atoms)
        else:
            raise ValueError(f'Unknown primitive type: {primitive_type}. Supported types are "conventional" and "primitive".')
    ...
    def run_relax_multi(self):
        ...
        """
        deformed = generate_deformed_structures(self.ctx.reference_structure,
    ...
    def compose_elastic_tensor(self):
        ...
        self.out('elastic_tensor', get_elastic_tensor(deform_datas=self.ctx.deformations, **miscs))
    

@calcfunction
def generate_deformed_structures(structure: orm.StructureData,
    ...
    deformed_structures = DeformedStructureSet(structure.get_pymatgen(),
